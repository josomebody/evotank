...
class console:
    ...
    def blit(self, screen, font):
        for i in range(len(self.consolemsg)):
            self.consolepic.append(font.render(self.consolemsg[i], True, (255, 128 / (6 - i) , 0)))
...
class tank: 
    ...
    y = 0
    moveable = 1
    rotate = 0 #0 = north, 1=northeast, 2=east, 3=southeast, 4=south, 5=southwest, 6=west, 7=northwest
    velocity = 0
    heat = 0
    armor = maxarmor
    ammo = maxammo
    ...
    def shoot (self, bang):
        ...
        if self.ammo > 0:
            ...
            if bang != 0:
                bangplay = 'bang.play(0)'
            else:
                bangplay = 'print \'shot fired\''
            exec(bangplay)
            return bullet
        else:
            return -1
...
def bounce(guy1, guy2):
    if guy2.moveable == 1:
        guy1.armor -= guy2.velocity
    if guy1.moveable == 1:
        guy2.armor -= guy1.velocity
    if guy1.x > guy2.x:
        if guy1.moveable == 1:
           guy1.x += 1
        if guy2.moveable == 1:
    ...
    if guy1.y < guy2.y:
        ...
        if guy2.moveable == 1:
           guy2.y += 1
    if detect_collision(guy1, tanksize, guy2, tanksize) == 1:
        bounce(guy1, guy2)
    else:
        if guy1.moveable == 1:
           guy1.velocity = 0
